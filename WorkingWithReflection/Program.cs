...
foreach (Type type in types)
{
    ...
    foreach (MemberInfo member in members)
    {
        ...
        List<CompilerGeneratedAttribute> compilerGeneratedAtributes = member.GetCustomAttributes<CompilerGeneratedAttribute>().ToList();

        if (compilerGeneratedAtributes.Count > 0)
        {
            return;
        }
